...
RUN git clone -b ${DOLFINX_VERSION} --single-branch --depth 1 https://github.com/fenics/basix.git && \
...
RUN RUN git clone -b ${DOLFINX_VERSION} --single-branch --depth 1 https://github.com/fenics/dolfinx.git && \
    cd dolfinx && \
...
# Install jupyter
RUN pip3 install --upgrade --no-cache-dir jupyter jupyterlab jupytext jupyterbook
